...
def ort_infer_check(onnx_model_path):
    r"""   
    onnxruntime should be 1.12.1 
    infer a qoperatormodel using ort
    """
...
# onnx_model = onnx.load("/home/wrq/yolov8_int8_.onnx")
# ort_outs = test_model_by_onnxruntime(onnx_model)

    
# ort_infer_check("after.onnx")
...
def onnx_qdq_to_qlinear(
    ...
    Args:
        model (ModelProto): fp32 qdq onnx model path or onnx file.
    """
    ...
    onnx.save(qdqQuantizer.model.model,"vit_after_opt.onnx")
    ...
    onnx.save(qdqQuantizer.model.model,"vit_int.onnx")
    # check onnx graph
    ...
    # check model in ort
    ort_infer_check(save_path)
    # check_model_in_ort(save_path)
    print("qoperator model is done")
    return qdqQuantizer.model.model
    
model = onnx_qdq_to_qlinear("/home/wrq/regnety_quant/regnet_quant_fx_reference_qdq_reference_shift_scale.onnx","/home/wrq/regnety_quant")
